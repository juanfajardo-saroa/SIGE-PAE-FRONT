...
@Component({
  selector: 'app-repositorios-circulares-uapa',
  templateUrl: './repositorios-circulares-uapa.component.html',
  styleUrls: ['./repositorios-circulares-uapa.component.scss']
})
export class RepositoriosCircularesUapaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private RepositoriosService: RepositoriosExtendService,
    public fechaPi: DatePipe, public dialog: MatDialog, private seguridadService: SeguridadService, private router: Router,
  ...
  direccionar() {
    this.router.navigate(['/RepositoriosCircularesUAPA']).then(() => {
  ...
  ngOnInit(): void {
    ...
    this._PA_repositoriosGetAllWithRelationService.getPA_repositoriosGetAllWithRelationList(6,Number(localStorage.getItem('VigSeleccionada'))).subscribe(
      (response: any) => {
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogRepositoriosCircularesUAPAContent, {
      data: obj
    });
...
@Component({
  ...
  templateUrl: 'repositorios-circulares-uapa.dialog.component.html',
  styleUrls: ["./repositorios-circulares-uapa.dialog.component.scss"],
})
// tslint:disable-next-line - Disables all
export class DialogRepositoriosCircularesUAPAContent {
  action: string;
  // tslint:disable-next-line - Disables all
  local_data: any;
  selectedImage: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogRepositoriosCircularesUAPAContent>,
  ...
  todosLosFiltros() {
    this.categoriasService.getCategoriasList().subscribe(
      (response: any) => {
        this.CategoriasList = response.filter(item => item.id === 6);
      },
      (err) => {
      }
