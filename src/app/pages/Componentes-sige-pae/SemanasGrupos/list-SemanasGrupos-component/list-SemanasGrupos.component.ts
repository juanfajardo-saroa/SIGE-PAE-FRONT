 ...
 * 
 */
import { SemanasGruposModel } from "src/app/shared/model/SemanasGrupos";
import { SemanasGruposService } from "src/app/shared/services/SemanasGrupos.services";
/**
 ...
 */
@Component({
    selector: "app-list-SemanasGrupos",
    templateUrl: "./list-SemanasGrupos.component.html",
    styleUrls: ["./list-SemanasGrupos.component.scss"],
  })
 ...
 *  
 */
export class ListSemanasGruposComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    SemanasGruposDetail : SemanasGruposModel | null=null;
    SemanasGruposList : SemanasGruposModel[]=[];
    currentYear = new Date().getFullYear();
    dateToday: number = Date.now();
    nombreETC =environment.nameETC;

    private dataArray: any;
    displayedColumns: string[] = ['select',"id","iD_Semana","siD_Semana","iD_GrupoAlimento","cantidadGrupo","auditoria", 'action'];
    public dataSource!: MatTableDataSource<SemanasGruposModel>;
    selection = new SelectionModel<SemanasGruposModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public SemanasGruposService: SemanasGruposService) { }
      ...
      */
    ngOnInit(): void {
        this.SemanasGruposService.getSemanasGruposList().subscribe(
            (response: any) => {
              this.dataArray = response;
              this.isLoading = false;
              this.dataSource = new MatTableDataSource<SemanasGruposModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogSemanasGruposContent, {
              data: obj
          });
      ...
      addRowData(row_obj: SemanasGruposModel ): void {
          this.SemanasGruposService.addSemanasGrupos(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: SemanasGruposModel ): boolean | any {
          this.SemanasGruposService.updateSemanasGrupos(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: SemanasGruposModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.SemanasGruposService.deleteSemanasGrupos(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-SemanasGrupos.dialog.component.html',
    styleUrls: ["./list-SemanasGrupos.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogSemanasGruposContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
    ...
    constructor(public dialogRef: MatDialogRef<DialogSemanasGruposContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: SemanasGruposModel,
...
iD_GrupoAlimento:[data.iD_GrupoAlimento],
cantidadGrupo:[data.cantidadGrupo,Validators.required],
auditoria:[''],
});
