 ...
 */
@Component({
    selector: "app-list-MenuPreparaciones",
    templateUrl: "./list-MenuPreparaciones.component.html",
    styleUrls: ["./list-MenuPreparaciones.component.scss"],
  })
 ...
 *  
 */
export class ListMenuPreparacionesComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    MenuPreparacionesDetail : MenuPreparacionesModel | null=null;
    MenuPreparacionesList : MenuPreparacionesModel[]=[];
    currentYear = new Date().getFullYear();
    dateToday: number = Date.now();
    nombreETC =environment.nameETC;

    private dataArray: any;
    displayedColumns: string[] = ['select',"id","iD_Preparacion","siD_Preparacion","iD_Menu","siD_Menu","auditoria", 'action'];
    public dataSource!: MatTableDataSource<MenuPreparacionesModel>;
    selection = new SelectionModel<MenuPreparacionesModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public MenuPreparacionesService: MenuPreparacionesService) { }
      ...
      */
    ngOnInit(): void {
        this.MenuPreparacionesService.getMenuPreparacionesList().subscribe(
            (response: any) => {
              this.dataArray = response;
              this.isLoading = false;
              this.dataSource = new MatTableDataSource<MenuPreparacionesModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogMenuPreparacionesContent, {
              data: obj
          });
      ...
      addRowData(row_obj: MenuPreparacionesModel ): void {
          this.MenuPreparacionesService.addMenuPreparaciones(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: MenuPreparacionesModel ): boolean | any {
          this.MenuPreparacionesService.updateMenuPreparaciones(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: MenuPreparacionesModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.MenuPreparacionesService.deleteMenuPreparaciones(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-MenuPreparaciones.dialog.component.html',
    styleUrls: ["./list-MenuPreparaciones.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogMenuPreparacionesContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
...
PreparacionesList: PreparacionesModel[];
    ...
    constructor(public dialogRef: MatDialogRef<DialogMenuPreparacionesContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: MenuPreparacionesModel,
