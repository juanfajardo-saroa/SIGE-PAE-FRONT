 ...
 * 
 */
import { AporteSemanasModel } from "src/app/shared/model/AporteSemanas";
import { AporteSemanasService } from "src/app/shared/services/AporteSemanas.services";
/**
 ...
 * 
 */
import { SemanasModel } from "src/app/shared/model/Semanas"; 
import { SemanasService } from "src/app/shared/services/Semanas.services"; 
 ...
 */
@Component({
    selector: "app-list-AporteSemanas",
    templateUrl: "./list-AporteSemanas.component.html",
    styleUrls: ["./list-AporteSemanas.component.scss"],
  })
 ...
 *  
 */
export class ListAporteSemanasComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    AporteSemanasDetail : AporteSemanasModel | null=null;
    AporteSemanasList : AporteSemanasModel[]=[];
    currentYear = new Date().getFullYear();
    dateToday: number = Date.now();
    nombreETC =environment.nameETC;

    private dataArray: any;
    displayedColumns: string[] = ['select',"id","iD_Semana","siD_Semana","iD_Nutriente","iD_TipoNivelEducativo","porcentajeAdecuacion","aporteEstimado","auditoria", 'action'];
    public dataSource!: MatTableDataSource<AporteSemanasModel>;
    selection = new SelectionModel<AporteSemanasModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public AporteSemanasService: AporteSemanasService) { }
      ...
      */
    ngOnInit(): void {
        this.AporteSemanasService.getAporteSemanasList().subscribe(
            (response: any) => {
              this.dataArray = response;
              this.isLoading = false;
              this.dataSource = new MatTableDataSource<AporteSemanasModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogAporteSemanasContent, {
              data: obj
          });
      ...
      addRowData(row_obj: AporteSemanasModel ): void {
          this.AporteSemanasService.addAporteSemanas(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: AporteSemanasModel ): boolean | any {
          this.AporteSemanasService.updateAporteSemanas(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: AporteSemanasModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.AporteSemanasService.deleteAporteSemanas(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-AporteSemanas.dialog.component.html',
    styleUrls: ["./list-AporteSemanas.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogAporteSemanasContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
    ...
    // Listas relacionales
    SemanasList: SemanasModel[];



    constructor(public dialogRef: MatDialogRef<DialogAporteSemanasContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: AporteSemanasModel,
        private fb:FormBuilder 
        , private semanasService:SemanasService

        ) {
   
        this.semanasService.getSemanasList().subscribe(
    (response:any) => {
        this.SemanasList = response;
    },
    (err)=>{
        console.log("Error al cargar los registros del selected de las Semanas", err);
    }
 );
...
porcentajeAdecuacion:[data.porcentajeAdecuacion,Validators.required],
aporteEstimado:[data.aporteEstimado,Validators.required],
auditoria:[''],
});
