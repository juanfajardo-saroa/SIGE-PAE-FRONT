 ...
 * 
 */
import { PreguntasModel } from "src/app/shared/model/Preguntas";
import { PreguntasService } from "src/app/shared/services/Preguntas.services";
/**
 ...
 * 
 */
import { CuestionariosModel } from "src/app/shared/model/Cuestionarios"; 
import { CuestionariosService } from "src/app/shared/services/Cuestionarios.services"; 
 ...
 *  
 */
export class ListPreguntasComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    PreguntasDetail : PreguntasModel | null=null;
    ...
    displayedColumns: string[] = ['select',"id","iD_Cuestionario","siD_Cuestionario","iD_ETC","siD_ETC","enunciado","orden","auditoria", 'action'];
    public dataSource!: MatTableDataSource<PreguntasModel>;
    selection = new SelectionModel<PreguntasModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public PreguntasService: PreguntasService) { }
      ...
      */
    ngOnInit(): void {
        this.PreguntasService.getPreguntasList().subscribe(
            (response: any) => {
              this.dataArray = response;
              this.isLoading = false;
              this.dataSource = new MatTableDataSource<PreguntasModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogPreguntasContent, {
              data: obj
          });
      ...
      addRowData(row_obj: PreguntasModel ): void {
          this.PreguntasService.addPreguntas(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: PreguntasModel ): boolean | any {
          this.PreguntasService.updatePreguntas(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: PreguntasModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.PreguntasService.deletePreguntas(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-Preguntas.dialog.component.html',
    styleUrls: ["./list-Preguntas.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogPreguntasContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
    ...
    // Listas relacionales
    CuestionariosList: CuestionariosModel[];
ETCList: ETCModel[];



    constructor(public dialogRef: MatDialogRef<DialogPreguntasContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: PreguntasModel,
...
, private eTCService:ETCService
        ...
        this.cuestionariosService.getCuestionariosList().subscribe(
    (response:any) => {
        this.CuestionariosList = response;
    },
    (err)=>{
        console.log("Error al cargar los registros del selected de las Cuestionarios", err);
    }
 );
...
id:[data.id],
iD_Cuestionario:[data.iD_Cuestionario,Validators.required],
