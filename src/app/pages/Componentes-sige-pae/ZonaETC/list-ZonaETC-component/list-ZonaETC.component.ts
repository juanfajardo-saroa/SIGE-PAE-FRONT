 ...
 * 
 */
import { ZonaETCModel } from "src/app/shared/model/ZonaETC";
import { ZonaETCService } from "src/app/shared/services/ZonaETC.services";
/**
 ...
 */
@Component({
  selector: "app-list-ZonaETC",
  templateUrl: "./list-ZonaETC.component.html",
  styleUrls: ["./list-ZonaETC.component.scss"],
})
...
*  
*/
export class ListZonaETCComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ZonaETCDetail: ZonaETCModel | null = null;
  ZonaETCList: ZonaETCModel[] = [];
  currentYear = new Date().getFullYear();
  dateToday: number = Date.now();
  nombreETC = environment.nameETC;

  private dataArray: any;
  displayedColumns: string[] = ["id", "siD_ETC", "siD_CentroAcopio", "nombre", 'action'];
  public dataSource!: MatTableDataSource<ZonaETCModel>;
  selection = new SelectionModel<ZonaETCModel>(true, []);
  ...
  constructor(public dialog: MatDialog, private route: ActivatedRoute, public ZonaETCService: ZonaETCService,
   ...
   */
  ngOnInit(): void {
    this.ZonaETCService.getZonaETCListRelation().subscribe(
      (response: any) => {
        this.dataArray = response;
        this.isLoading = false;
        this.dataSource = new MatTableDataSource<ZonaETCModel>(this.dataArray);
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogZonaETCContent, {
      data: obj
    });
  ...
  addRowData(row_obj: ZonaETCModel): void {
    this.ZonaETCService.addZonaETC(row_obj).subscribe(
      (response) => {
  ...
  updateRowData(row_obj: ZonaETCModel): boolean | any {
    this.ZonaETCService.updateZonaETC(row_obj).subscribe(
      (response) => {
  ...
  deleteRowData(row_obj: ZonaETCModel): boolean | any {
    const ideliminar = row_obj.id;
    const nombre= row_obj.nombre
    console.log(row_obj);
    
    this.ZonaETCService.deleteZonaETC(ideliminar,nombre).subscribe(
      (response) => {
...
@Component({
  ...
  templateUrl: 'list-ZonaETC.dialog.component.html',
  styleUrls: ["./list-ZonaETC.dialog.component.scss"],
})
...
*  
*/
export class DialogZonaETCContent {
  action: string;
  local_data: any;
  selectedImage: any = '';
  joiningDate: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogZonaETCContent>,
    @Optional() @Inject(MAT_DIALOG_DATA) public data: ZonaETCModel,
    ...
    this.centroDeAcopioService.getCentroDeAcopioList().subscribe(
      (response: any) => {
        this.CentroDeAcopioList = response;
      },
      (err) => {
        console.log("Error al cargar los registros del selected de las CentroDeAcopio", err);
      }
    );
    ...
    this.form = this.fb.group({
      ...
      iD_CentroAcopio: [data.iD_CentroAcopio, Validators.required],
