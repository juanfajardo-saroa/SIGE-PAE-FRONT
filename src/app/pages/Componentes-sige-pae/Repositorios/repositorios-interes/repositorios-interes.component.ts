...
@Component({
  selector: 'app-repositorios-interes',
  templateUrl: './repositorios-interes.component.html',
  styleUrls: ['./repositorios-interes.component.scss']
})
export class RepositoriosInteresComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this._PA_repositoriosGetAllWithRelationService.getPA_repositoriosGetAllWithRelationList(8,Number(localStorage.getItem('VigSeleccionada'))).subscribe(
      (response: any) => {
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogRepositorioInteresContent, {
      data: obj
    });
...
@Component({
  ...
  templateUrl: 'repositorios-interes.dialog.component.html',
  styleUrls: ["./repositorios-interes.dialog.component.scss"],
})
// tslint:disable-next-line - Disables all
export class DialogRepositorioInteresContent {
  action: string;
  // tslint:disable-next-line - Disables all
  local_data: any;
  selectedImage: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogRepositorioInteresContent>,
  ...
  todosLosFiltros() {
    this.categoriasService.getCategoriasList().subscribe(
      (response: any) => {
        this.CategoriasList = response.filter(item => item.id === 8);
      },
      (err) => {
      }
