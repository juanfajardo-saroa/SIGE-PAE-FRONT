...
@Component({
  selector: 'app-repositorios-documentos',
  templateUrl: './repositorios-documentos.component.html',
  styleUrls: ['./repositorios-documentos.component.scss']
})
export class RepositoriosDocumentosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private RepositoriosService: RepositoriosExtendService, 
  ...
  ngOnInit(): void {
    this._PA_repositoriosGetAllWithRelationService.getPA_repositoriosGetAllWithRelationList(3,Number(localStorage.getItem('VigSeleccionada'))).subscribe(
      (response: any) => {
        console.log(response);
       
        this.RepositoriosList =response;

        console.log(this.RepositoriosList);
        for (let repositorio in this.RepositoriosList) {
          let _fileUpload: fileUploadModel;
          _fileUpload = { file: null, fileName: this.RepositoriosList[repositorio].pahtArchivo, cnx: environment.cnxBS, container: environment.containerBS };
          // console.log('archivo',_fileUpload);
          const fileupload = _fileUpload as unknown as File;
          ...
          //console.log('Diferencia', fecha1a, fecha2);
          let diferncia = (fecha2.getTime() - fecha1a.getTime()) / 1000;
          diferncia /= (60 * 60);

          //console.log(Math.abs(Math.round(diferncia)));
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogRepositorioDocumentosContent, {
      data: obj
    });
...
this.RepositoriosObject.fechaArchivo = row_obj.fechaArchivo;
    ...
    this.RepositoriosObject.idTipoArchivo = row_obj.idTipoArchivo;
...
@Component({
  ...
  templateUrl: 'repositorios-documentos.dialog.component.html',
  styleUrls: ["./repositorios-documentos.dialog.component.scss"],
})
// tslint:disable-next-line - Disables all
export class DialogRepositorioDocumentosContent {
  action: string;
  // tslint:disable-next-line - Disables all
  local_data: any;
  selectedImage: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogRepositorioDocumentosContent>,
  ...
  todosLosFiltros() {
    this.categoriasService.getCategoriasList().subscribe(
      (response: any) => {
        this.CategoriasList = response.filter(item => item.id === 3);
      },
      (err) => {
      }
