...
@Component({
  selector: 'app-repositorios-circulares',
  templateUrl: './repositorios-circulares.component.html',
  styleUrls: ['./repositorios-circulares.component.scss']
})
export class RepositoriosCircularesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogRepositoriosCircularesContent, {
      data: obj
    });
  ...
  deleteRowData(row_obj: RepositoriosExtendModel): boolean | any {
    ...
    this.RepositoriosService.deleteRepositorios(ideliminar,row_obj.pahtArchivo,row_obj.nombreResolucion).subscribe(
      ...
      (err) => {
         console.log( "No fue posible eliminar el registro, es posible que tenga dependencias hijas: <br>" +err);
      }
    );
...
@Component({
  ...
  templateUrl: 'repositorios-circulares.dialog.component.html',
  styleUrls: ["./repositorios-circulares.dialog.component.scss"],
})
// tslint:disable-next-line - Disables all
export class DialogRepositoriosCircularesContent {
  action: string;
  // tslint:disable-next-line - Disables all
  local_data: any;
  selectedImage: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogRepositoriosCircularesContent>,
