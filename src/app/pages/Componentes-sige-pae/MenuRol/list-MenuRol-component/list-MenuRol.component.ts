 ...
 * 
 */
import { MenuRolModel } from "src/app/shared/model/MenuRol";
import { MenuRolService } from "src/app/shared/services/MenuRol.services";
/**
 ...
 */
@Component({
    selector: "app-list-MenuRol",
    templateUrl: "./list-MenuRol.component.html",
    styleUrls: ["./list-MenuRol.component.scss"],
  })
 ...
 *  
 */
export class ListMenuRolComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    MenuRolDetail : MenuRolModel | null=null;
    MenuRolList : MenuRolModel[]=[];
    currentYear = new Date().getFullYear();
    dateToday: number = Date.now();
    nombreETC =environment.nameETC;

    private dataArray: any;
    displayedColumns: string[] = ['select',"id","id_Menu","aspNetRoles","auditoria", 'action'];
    public dataSource!: MatTableDataSource<MenuRolModel>;
    selection = new SelectionModel<MenuRolModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public MenuRolService: MenuRolService) { }
      ...
      */
    ngOnInit(): void {
        this.MenuRolService.getMenuRolList().subscribe(
            (response: any) => {
              this.dataArray = response;
              this.isLoading = false;
              this.dataSource = new MatTableDataSource<MenuRolModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogMenuRolContent, {
              data: obj
          });
      ...
      addRowData(row_obj: MenuRolModel ): void {
          this.MenuRolService.addMenuRol(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: MenuRolModel ): boolean | any {
          this.MenuRolService.updateMenuRol(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: MenuRolModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.MenuRolService.deleteMenuRol(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-MenuRol.dialog.component.html',
    styleUrls: ["./list-MenuRol.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogMenuRolContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
    ...
    constructor(public dialogRef: MatDialogRef<DialogMenuRolContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: MenuRolModel,
...
id:[data.id],
id_Menu:[data.id_Menu,Validators.required],
aspNetRoles:[data.aspNetRoles,Validators.required],
auditoria:[''],
});
