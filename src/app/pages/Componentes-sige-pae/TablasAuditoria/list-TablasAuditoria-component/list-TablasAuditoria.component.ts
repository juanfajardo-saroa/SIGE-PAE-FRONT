...
import { TablasAuditoriaModel } from 'src/app/shared/model/TablasAuditoria';
import { TablasAuditoriaService } from "src/app/shared/services/TablasAuditoria.services";
/**
 ...
 */
@Component({
  selector: "app-list-TablasAuditoria",
  templateUrl: "./list-TablasAuditoria.component.html",
  styleUrls: ["./list-TablasAuditoria.component.scss"],
})
...
*
*/
export class ListTablasAuditoriaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private apiurl = environment.baseUrlAPI_Auditoria + "TablasAuditoria";
  TablasAuditoriaDetail: TablasAuditoriaModel | null = null;
  TablasAuditoriaList: TablasAuditoriaModel[] = [];
  currentYear = new Date().getFullYear();
  dateToday: number = Date.now();
  nombreETC = environment.nameETC;

  private dataArray: any;
  displayedColumns: string[] = [ "id", "esquema", "table_Name", "nombre", "auditCampo", "auditTabla", 'action'];
  public dataSource!: MatTableDataSource<TablasAuditoriaModel>;
  selection = new SelectionModel<TablasAuditoriaModel>(true, []);
  ...
  constructor(public dialog: MatDialog, private route: ActivatedRoute, public TablasAuditoriaService: TablasAuditoriaService) { }
   ...
   */
  ngOnInit(): void {
    this.TablasAuditoriaService.getTablasAuditoriaList().subscribe(
      (response: any) => {
        this.dataArray = response;
        this.isLoading = false;
        this.dataSource = new MatTableDataSource<TablasAuditoriaModel>(this.dataArray);
  ...
  openDialog(action: string, obj: any): void {
    obj.action = action;
    const dialogRef = this.dialog.open(DialogTablasAuditoriaContent, {
      data: obj
    });
  ...
  addRowData(row_obj: TablasAuditoriaModel): void {
    this.TablasAuditoriaService.addTablasAuditoria(row_obj).subscribe(
      (response) => {
  ...
  updateRowData(row_obj: TablasAuditoriaModel): boolean | any {
    this.TablasAuditoriaService.updateTablasAuditoria(row_obj).subscribe(
      (response) => {
  ...
  deleteRowData(row_obj: TablasAuditoriaModel): boolean | any {
    const ideliminar = row_obj.id;
    this.TablasAuditoriaService.deleteTablasAuditoria(ideliminar).subscribe(
      (response) => {
...
@Component({
  ...
  templateUrl: 'list-TablasAuditoria.dialog.component.html',
  styleUrls: ["./list-TablasAuditoria.dialog.component.scss"],
})
...
*
*/
export class DialogTablasAuditoriaContent {
  action: string;
  local_data: any;
  selectedImage: any = '';
  joiningDate: any = '';
  ...
  constructor(public dialogRef: MatDialogRef<DialogTablasAuditoriaContent>,
    @Optional() @Inject(MAT_DIALOG_DATA) public data: TablasAuditoriaModel,
    ...
    this.form = this.fb.group({
      ...
      nombre: [data.nombre],
      auditoria: [''],
      auditCampo: [data.auditCampo ? data.auditCampo:false, Validators.required],
      auditTabla: [data.auditTabla ? data.auditTabla:false, Validators.required],
    });
