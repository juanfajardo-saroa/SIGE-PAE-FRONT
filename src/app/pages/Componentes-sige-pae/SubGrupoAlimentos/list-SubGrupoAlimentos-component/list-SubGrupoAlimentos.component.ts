 ...
 */
@Component({
    selector: "app-list-SubGrupoAlimentos",
    templateUrl: "./list-SubGrupoAlimentos.component.html",
    styleUrls: ["./list-SubGrupoAlimentos.component.scss"],
  })
 ...
 *  
 */
export class ListSubGrupoAlimentosComponent implements OnInit, AfterViewInit,OnDestroy  {
    ...
    SubGrupoAlimentosDetail : SubGrupoAlimentosModel | null=null;
    ...
    displayedColumns: string[] = ['select',"id","iD_GrupoAlimento","siD_GrupoAlimento","nombre","auditoria", 'action'];
    public dataSource!: MatTableDataSource<SubGrupoAlimentosModel>;
    selection = new SelectionModel<SubGrupoAlimentosModel >(true, []);
    ...
    constructor(public dialog: MatDialog,  private route : ActivatedRoute, public SubGrupoAlimentosService: SubGrupoAlimentosService) { }
    ...
    ngOnInit(): void {
        this.SubGrupoAlimentosService.getSubGrupoAlimentosList().subscribe(
            (response: any) => {
              ...
              this.dataSource = new MatTableDataSource<SubGrupoAlimentosModel>(this.dataArray);
    ...
    ngOnDestroy() {
      ...
      */
      ...
      openDialog(action: string, obj: any): void {
          obj.action = action;
          const dialogRef = this.dialog.open(DialogSubGrupoAlimentosContent, {
              data: obj
          });
      ...
      addRowData(row_obj: SubGrupoAlimentosModel ): void {
          this.SubGrupoAlimentosService.addSubGrupoAlimentos(row_obj).subscribe(
              (response) => {
      ...
      updateRowData(row_obj: SubGrupoAlimentosModel ): boolean | any {
          this.SubGrupoAlimentosService.updateSubGrupoAlimentos(row_obj).subscribe(
              (response) => {
      ...
      deleteRowData(row_obj: SubGrupoAlimentosModel ): boolean | any {
          const ideliminar= row_obj.id;
          this.SubGrupoAlimentosService.deleteSubGrupoAlimentos(ideliminar).subscribe(
            (response) => {
...
@Component({
    ...
    templateUrl: 'list-SubGrupoAlimentos.dialog.component.html',
    styleUrls: ["./list-SubGrupoAlimentos.dialog.component.scss"],
})
 ...
 *  
 */
export class DialogSubGrupoAlimentosContent {
    action: string;
    local_data: any;
    selectedImage: any = '';
    joiningDate: any = '';
    ...
    constructor(public dialogRef: MatDialogRef<DialogSubGrupoAlimentosContent>,
        @Optional() @Inject(MAT_DIALOG_DATA) public data: SubGrupoAlimentosModel,
        private fb:FormBuilder 
        , private grupoAlimentosService:GrupoAlimentosService

        ) {
   
        this.grupoAlimentosService.getGrupoAlimentosList().subscribe(
    (response:any) => {
        this.GrupoAlimentosList = response;
    },
    (err)=>{
        console.log("Error al cargar los registros del selected de las GrupoAlimentos", err);
    }
 );
