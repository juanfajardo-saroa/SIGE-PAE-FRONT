...
import { PA_conteoIntercambiosPiv, } from 'src/app/shared/services/PA_conteoIntercambios.services';
...
@Component({
  selector: 'app-ptn-disp-ver-cicl',
  templateUrl: './ptn-disp-ver-cicl.component.html',
  styleUrls: ['./ptn-disp-ver-cicl.component.scss']
})
export class PtnDispVerCiclComponent implements OnInit {
  ...
  PA_conteoIntercambiosPivreq: PA_conteoIntercambiosPiv = {}
  preg: boolean = false;
  preg1: boolean = false;
  preg2: boolean = false;
  preg5: boolean = false;
  ...
  RegresarAprobaciones() {
    ...
    this.router.navigate(['/PTNCiclosDeMenu'], { queryParams: { tab: 0 } })
  }
  ...
  */
  ...
  MacroMicro() {
    ...
    if (this.CiclosMenusObject[0].iD_TipoModeloOperacion == 1) {
      ...
      if (this.diasList == undefined) { } else {
        let f = this.diasList.filter(item => item.nombre == this.diasText)
        if (f.length == 0) { } else {
          dia = f[0].numeroDia
        }
  ...
  fillTableAprobaciones() {
    this.AccionesAprobacionService.getAccionesAprobacionList().subscribe(
      (response: any) => {
        ...
        this._PA_AprobacionesGetAllFullService.getPA_AprobacionesGetAllFullList(18).subscribe(
          (response: any) => {
            this.UsersList = response;
            this.serviciosp.getGetAprobacionesGetAllWithRelListfilter(18, this.idETC).subscribe(
              (response: any) => {
