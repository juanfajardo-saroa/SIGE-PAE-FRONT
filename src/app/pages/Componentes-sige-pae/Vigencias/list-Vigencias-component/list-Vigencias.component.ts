...
import * as moment from 'moment';
  ...
  */
 @Component({
   selector: "app-list-Vigencias",
   templateUrl: "./list-Vigencias.component.html",
   styleUrls: ["./list-Vigencias.component.scss"],
 })
 ...
 *
 */
 export class ListVigenciasComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   private apiurl = environment.baseUrlAPI + "Vigencias";
   VigenciasDetail: VigenciasModel | null = null;
   ...
   displayedColumns: string[] = [ "id", "nombre", "ano", "fechaInicial", "vigenciaActual"];
   public dataSource!: MatTableDataSource<VigenciasModel>;
   selection = new SelectionModel<VigenciasModel>(true, []);
   ...
   constructor(
     ...
     public VigenciasService: VigenciasService,
   ...
   ngOnInit(): void {
     this.VigenciasService.getVigenciasList().subscribe(
       (response: any) => {
         ...
         this.dataArray.find(element => {
          let p =  element.fechaInicial
           element.fechaInicial = moment(p).format('DD-MMM-yyyy').toUpperCase();

         });

         this.isLoading = false;
         this.dataSource = new MatTableDataSource<VigenciasModel>(this.dataArray);
 ...
 openDialog(action: string, obj: any): void {
   obj.action = action;
   const dialogRef = this.dialog.open(DialogVigenciasContent, {
     data: obj
   });
 ...
 addRowData(row_obj: VigenciasModel): void {
  //  console.log('loga1',row_obj);
   this.VigenciasService.addVigencias(row_obj).subscribe(
     (response) => {
 ...
 updateRowData(row_obj: VigenciasModel): boolean | any {
  // console.log('loga2',row_obj);
   this.VigenciasService.updateVigencias(row_obj).subscribe(
     (response) => {
 ...
 deleteRowData(row_obj: VigenciasModel): boolean | any {
   const ideliminar = row_obj.id;
   this.VigenciasService.deleteVigencias(ideliminar).subscribe(
     (response) => {
 ...
 @Component({
   ...
   templateUrl: 'list-Vigencias.dialog.component.html',
   styleUrls: ["./list-Vigencias.dialog.component.scss"],
 })
 ...
 *
 */
 export class DialogVigenciasContent {
   action: string;
   local_data: any;
   selectedImage: any = '';
   joiningDate: any = '';
   ...
   constructor(public dialogRef: MatDialogRef<DialogVigenciasContent>,
     @Optional() @Inject(MAT_DIALOG_DATA) public data: VigenciasModel,
     ...
     this.form = this.fb.group({
       ...
       vigenciaActual: [data.vigenciaActual ? data.vigenciaActual:false, Validators.required],
       auditoria: [''],
     });
