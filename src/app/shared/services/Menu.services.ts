 ...
 * Ano			    :2022
 *
 * Arquitectura	    :Modelo Base de Angular para Arquitectura Microservicios de la Entidad Menu
 *
...
/**
 *  decorador injectable indica que esta clase Menu puede ser inyectada dinamicamente
 *
 *
 */
...
export class MenuService {
  ...
  private apiurl = environment.baseUrlAPI_Seguridad + "Menu";
  private filterArray: MenuModel[] = [];
  private Menu: MenuModel[] = [];
  private MenuList: MenuModel[] = [];
  private MenuObject: MenuModel;
  ...
  private subMenu = new BehaviorSubject<any>({});
  subMenuListener$ = this.subMenu.asObservable();
  ...
  getMenuListFilter(sis:string): Observable<MenuModel> {
    const url = `${this.apiurl}/GetAll?filter=accion eq '${sis}'`;
    return this.http
      .get<MenuModel>(url)
      .pipe(
        tap(),   // para poder realizar efectos secundrios
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  getMenuListFull(): Observable<MenuModel> {
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  getMenuListRelation(): Observable<MenuModel> {
  ...
  getMenu(id: number): Observable<MenuModel> {
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  deleteMenu(id: number, nombre:string,link:string,grupo:string,controlador:string,accion:string,orden:number,ordenpadre:number,padre:number): Observable<MenuModel> {

    var MenuObject = new MenuModel(id,nombre,controlador,accion,grupo,link,orden,ordenpadre,padre,'',false,'',
    ...
    return this.http
      .post<MenuModel>(url, JSON.stringify(MenuObject),this.httpOptions)
      .pipe(
  ...
  addMenu(Menu: MenuModel): Observable<MenuModel> {
    Menu.id = 0;
    Menu.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
    ...
    Menu._usuario=localStorage.getItem('NombreUsuario');
    Menu._accion="Adicionar";
    Menu._ippublica=localStorage.getItem('IpPublica');
    Menu._browser=localStorage.getItem("Browser");
    Menu._nombremaquina=localStorage.getItem("NombreMaquina");
    Menu._sessionid=localStorage.getItem("Ubicacion");

    const url = `${this.apiurl}/Post/`;
    return this.http
      .post<MenuModel>(url, JSON.stringify(Menu),this.httpOptions)
      .pipe(
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  updateMenu(Menu: MenuModel): Observable<MenuModel> {
    Menu.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
    "{'RolPersonalizado':'"+          localStorage.getItem("RolPersonalizado")+"'},"+
    "{'NombreUsuario':'"+        localStorage.getItem("NombreUsuario")+"'},"+
    "{'Ubicacion':'"+    localStorage.getItem("Ubicacion")+"'},"+
    ...
    return this.http
      .put<MenuModel>(url, JSON.stringify(Menu),this.httpOptions)
      .pipe(
  ...
  eraseSubMenu() {
    this.subMenu = new BehaviorSubject<any>({});
  }
