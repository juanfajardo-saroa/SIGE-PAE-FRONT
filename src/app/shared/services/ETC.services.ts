...
/**
 *  decorador injectable indica que esta clase ETC puede ser inyectada dinamicamente
 *
 *  
 */
  ...
  *  
  */
 import { ETCModel } from "../model/ETC";
 ...
 export class ETCService {
   ...
   private apiurl = environment.baseUrlAPI_ETC + "ETC";
   private filterArray: ETCModel[] = [];
   private ETC: ETCModel[] = [];
   private ETCList: ETCModel[] = [];
   private ETCObject: ETCModel;
   ...
   private handleError(error: any) {
     ...
     */
   getETCList(): Observable<ETCModel> {
   ...
   getETCListfilter(nom:string): Observable<ETCModel> {
    const url = `${this.apiurl}/GetAll?%24filter=nombre%20eq%20%20%27${nom}%27`;
    return this.http
      .get<ETCModel>(url)
      .pipe(
        tap(),   // para poder realizar efectos secundrios
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   getETCListFull(): Observable<ETCModel> {
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   getETCListRelation(): Observable<ETCModel> {
   ...
   getETCListRelationfilter(id:number): Observable<ETCModel> {
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   getETC(id: number): Observable<ETCModel> {
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   deleteETC(id: number): Observable<ETCModel> {
 
     var ETCObject = new ETCModel(id,0,'','','',0,'','',
     ...
     return this.http
       .post<ETCModel>(url, JSON.stringify(ETCObject),this.httpOptions)
       .pipe(
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   addETC(ETC: ETCModel): Observable<ETCModel> {
     ETC.id = 0;
     ETC.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
     ...
     ETC._usuario=localStorage.getItem('NombreUsuario');
     ETC._accion="Adicionar";
     ETC._ippublica=localStorage.getItem('IpPublica');
     ETC._browser=localStorage.getItem("Browser");
     ETC._nombremaquina=localStorage.getItem("NombreMaquina");
     ETC._sessionid=localStorage.getItem("Ubicacion");
 
     const url = `${this.apiurl}/Post/`;
     return this.http
       .post<ETCModel>(url, JSON.stringify(ETC),this.httpOptions)
       .pipe(
     ...
     *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
     */
   updateETC(ETC: ETCModel): Observable<ETCModel> {
     ETC.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
     "{'RolPersonalizado':'"+          localStorage.getItem("RolPersonalizado")+"'},"+
     "{'NombreUsuario':'"+        localStorage.getItem("NombreUsuario")+"'},"+
     "{'Ubicacion':'"+    localStorage.getItem("Ubicacion")+"'},"+
     ...
     return this.http
       .put<ETCModel>(url, JSON.stringify(ETC),this.httpOptions)
       .pipe(
