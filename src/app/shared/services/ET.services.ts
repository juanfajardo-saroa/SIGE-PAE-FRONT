 ...
 * Ano			    :2022
 *
 * Arquitectura	    :Modelo Base de Angular para Arquitectura Microservicios de la Entidad ET
 *
...
/**
 *  decorador injectable indica que esta clase ET puede ser inyectada dinamicamente
 *
 *
 */
...
export class ETService {
  ...
  private apiurl = environment.baseUrlAPI_ETC + "ET";
  private filterArray: ETModel[] = [];
  private ET: ETModel[] = [];
  private ETList: ETModel[] = [];
  private ETObject: ETModel;
  ...
  getETListRelationFilter(id_ie:number): Observable<ETModel> {
    const url = `${this.apiurl}/GetAll?%24filter=iD_ETC%20eq%20${id_ie}`;
    return this.http
      .get<ETModel>(url)
      .pipe(
        tap(),   // para poder realizar efectos secundrios
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  getETListFull(): Observable<ETModel> {
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  getETListRelation(): Observable<ETModel> {
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  getET(id: number): Observable<ETModel> {
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  deleteET(id: number): Observable<ETModel> {

    var ETObject = new ETModel(id,0,'','','',0,'','',
    ...
    return this.http
      .post<ETModel>(url, JSON.stringify(ETObject),this.httpOptions)
      .pipe(
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  addET(ET: ETModel): Observable<ETModel> {
    ET.id = 0;
    ET.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
    ...
    ET._usuario=localStorage.getItem('NombreUsuario');
    ET._accion="Adicionar";
    ET._ippublica=localStorage.getItem('IpPublica');
    ET._browser=localStorage.getItem("Browser");
    ET._nombremaquina=localStorage.getItem("NombreMaquina");
    ET._sessionid=localStorage.getItem("Ubicacion");

    const url = `${this.apiurl}/Post/`;
    return this.http
      .post<ETModel>(url, JSON.stringify(ET),this.httpOptions)
      .pipe(
    ...
    *   Como esta clase esta soportada sobre dapper permite realizar filtros en formato LINQ
    */
  updateET(ET: ETModel): Observable<ETModel> {
    ET.auditoria ="{'RolBase':'"+localStorage.getItem("RolBase")+"'},"+
    "{'RolPersonalizado':'"+          localStorage.getItem("RolPersonalizado")+"'},"+
    "{'NombreUsuario':'"+        localStorage.getItem("NombreUsuario")+"'},"+
    "{'Ubicacion':'"+    localStorage.getItem("Ubicacion")+"'},"+
    ...
    return this.http
      .put<ETModel>(url, JSON.stringify(ET),this.httpOptions)
      .pipe(
